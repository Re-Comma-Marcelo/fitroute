...
function RouteLine({ onInk = false, className }: { onInk?: boolean; className?: string }) {
  return (
    <span className={cn("relative block h-6 w-24", className)} aria-hidden="true">
      ...
      <Stop size="divider" className={cn("absolute -right-1 top-[11px]", onInk && "bg-oxide-light")} />
    </span>
  );
}
...
export function RouteLogo({
  ...
  if (variant === "icon") {
    return (
      <span className={cn("grid size-12 place-items-center rounded-lg bg-ink", className)} aria-label="ROUTE">
  ...
  if (variant === "full-stop") {
    return (
      <span className={cn("inline-flex items-end gap-1 font-display text-xl font-extrabold uppercase", className)}>
        ROUTE<Stop size="corner" className="mb-1" />
      </span>
    );
  }
  ...
  if (variant === "masthead") {
    return (
      <span className={cn("block", className)}>
        ...
        <span className="mt-2 flex items-center"><span className="h-3.5 flex-1 bg-violet" /><Stop size="heavy" /></span>
      </span>
    );
  }
  return (
