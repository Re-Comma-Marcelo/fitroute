...
function groupByWorkout(
  ...
  const order = new Map(
    workouts.map((w) => [w.id, new Date(w.iniciadoEm).getTime()]),
  );
...
export function perWorkoutStats(
  sets: WorkoutSet[],
  workouts: Workout[],
): PerWorkoutStats[] {
...
export function isSameWeightForLastN(
  stats: PerWorkoutStats[],
  n: number,
): boolean {
  if (stats.length < n) return false;
  const last = stats.slice(-n);
  return last.every(
    (s) => s.maxWeight === last[0]!.maxWeight && s.maxWeight > 0,
  );
}

export function sessionsSinceWeightIncrease(
  stats: PerWorkoutStats[],
): number {
...
export function muscleGroupVolumeThisWeek(
  ...
  for (const s of sets) {
    const w = workouts.find((x) => x.id === s.workoutId);
    if (!w || new Date(w.iniciadoEm).toISOString() < start || s.tipoSerie === "aquecimento") continue;
