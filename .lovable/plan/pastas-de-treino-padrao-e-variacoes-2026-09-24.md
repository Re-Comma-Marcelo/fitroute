...
- **Fase 5 — feita.** "Encerrar ciclo" (pasta atual) e "Salvar como modelo" (pasta arquivada) no detalhe da pasta, em 3 passos: (1) **Trocas que ficaram** — trocas feitas em ≥50% das sessões de uma rotina padrão (mín. 2), marcadas por padrão, viram o padrão do modelo; (2) **Cargas de referência** — melhor série da última sessão de cada exercício, gravada nas notas da rotina numa linha `📌 Fim de {pasta}: 80 kg × 8` (substituída no próximo encerramento e mantida fora das notas de cada treino); (3) **Próximo ciclo** — nome sugerido ("… · 2"), a partir do modelo ou vazio, com opção de levar as variações. Um modelo pode iniciar uma nova pasta (detalhe do modelo ou "Nova pasta" → "Do modelo X") e continua editável.
- **Fase 6 — feita.** Troca sugerida (card do Coach, "Replace exercise" e chat do Coach na sessão) prioriza o substituto que você já usou no lugar daquele exercício na pasta atual. MCP: `get_training_context` devolve `trainingFolder` (rotinas padrão com exercícios, variações, sessões, trocas mais comuns com grupo muscular e motivos); `create_routine` cria na pasta atual e aceita `role` (`standard`/`variation`), `variationOf` e `reason` — atualizar sem `role` mantém o papel atual. Códigos ROUTE1 ganham os mesmos campos opcionais (códigos antigos continuam válidos) e a importação cria a variação ligada à rotina padrão. `manifest.json` atualizado.
