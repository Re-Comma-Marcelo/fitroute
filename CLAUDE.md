...
`src/lib/data/*.ts` is the only layer components should import from (one file per domain: `exercises`, `routines`, `folders`, `workouts`, `profile`, `body-weight`, `nutrition`, `meals.mock`, `week-menu`, `tracked-lifts`, `coaching`, `coach-notes`, `diet-entries`, `route`, `prices`, `claude-import`). Each exposes async functions with a stable signature. The built-in exercise/meal catalogs ship in `src/lib/data/mocks.ts`; per-user Supabase rows are merged on top (stored rows win by id) — see the `mergeCatalog` pattern in [src/lib/data/exercises.ts](src/lib/data/exercises.ts). Never import `mocks.ts` directly from a component. Routines and sessions are grouped into training folders (`src/lib/data/folders.ts`, migration `scripts/supabase-migration-folders.sql`): one current folder per user, routines are `padrao` (standard) or `variacao` (variation), sessions record `variacao`/`motivo`, and each set records the standard exercise it replaced (`substituiExerciseId`). Everything degrades to the flat, folder-less behavior when the migration hasn't been applied.
